/**
 * GET  /api/calendar  — publieke JSON { booked: string[] }
 * POST /api/calendar  — body { booked: string[] }, header Authorization: Bearer <CALENDAR_ADMIN_TOKEN>
 * Vereist op Vercel: Blob store (→ BLOB_READ_WRITE_TOKEN) + CALENDAR_ADMIN_TOKEN (eigen geheim).
 */
  ...
  if (req.method === "POST") {
    ...
    if (!token) {
      return sendJson(res, 503, {
        error: "Blob niet geconfigureerd. Voeg Vercel Blob toe (Storage) en koppel aan dit project.",
      });
    }
    ...
    if (!Array.isArray(body.booked)) {
      return sendJson(res, 400, { error: "Velden booked (array van YYYY-MM-DD) vereist" });
    }
